...
The site stays route-based like any website: home is `#/` (with `#/home` as an alias), and the informational pages `#/about`, `#/faq`, `#/terms`, `#/contact` are public too (no login gate). The guest shell is a small modern site — a sticky blurred **navbar** (logo → home, centre links *Home / About / Contact* with an active underline, Install pill + a single **Login** pill; the mark collapses to icon-only and Install becomes an icon button on phones), the routed page, then `SiteFooter`. Guests fetch discovery from the token-free twin endpoint `POST /api/salons/salon-list-public` (same payload as `salon-list`; signed-in customers keep the personalized `/salon-list`). Logged-in sessions never render the public navbar — `#/login` resolves to the dashboard home and only logout returns to public pages (`AppRoot` render branches on session, `getRouteFromHash` keeps role routes). All public pages end in the website footer (`SiteFooter` in `src/components/UserScreens.jsx`): brand, **Get the app** Google Play badge (`https://play.google.com/store/apps/details?id=com.mynaai`), a dashed **iOS — coming soon** badge with a note that the web app already works everywhere, quick links, the support number and the copyright bar. The PWA manifest uses `start_url: "/"` with standalone display, so an installed app with a saved session opens straight into the dashboard. Layout rails follow the shared 1460px `.screen` max-width with the responsive 1/2/3/4-column salon grid (phone → desktop).
...
| `mynaaiPendingRoute` | Session-scoped stash of the page a guest must resume after login (§3) |
