...
- **Location is optional and asked in context.** It powers distances and nearest-first sorting on discovery (the **Use my location** button), the salon address editor (**Allow location**) and salon signup. **Not now** on the login row is remembered, the app never nags, and the home list simply shows the unsorted salon list with an unobtrusive notice instead.
